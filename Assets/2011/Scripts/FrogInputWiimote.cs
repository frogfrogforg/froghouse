...
namespace Frog2011 {
    public class FrogInputWiimote : FrogInput {
        ...
        public override float GetDX() {
            if (showGui) return 0f;
            return Mathf.Clamp(roll*3f, -1f, 1f);
        }
    	...
    	void Update () {
            ...
            if (!IsWiimoteAvailable()) { return; }

            wiimote = WiimoteManager.Wiimotes[playerIndex];
           ...
           // Debug.Log(GetAccelVector());
            ...
            Vector3 accel = GetAccelVector(); // = direction of gravity, unless force is applied
            pitch = Mathf.Atan (accel.x/Mathf.Sqrt(accel.y*accel.y + accel.z*accel.z))/Mathf.PI;
            roll = Mathf.Atan (accel.z/Mathf.Sqrt(accel.x*accel.x + accel.y*accel.y))/Mathf.PI;
            forceMagnitude = accel.magnitude;
        ...
        void OnGUI()
        {
            if (showGui) {
                ...
                GUILayout.Label("Player: " + (playerIndex+1));
                GUILayout.Label("Wiimote Found: " + IsWiimoteAvailable());
                ...
                Vector3 v = GetAccelVector();
                GUILayout.Label("Accel Vector: "+v+" "+v.magnitude);
                if (v.magnitude > 3f) {
                    Debug.Log("SHAKE!  "+v.magnitude);
                } 
