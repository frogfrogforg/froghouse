...
{  
    ...
    public Transform gunPoint; //Assign the first 
    ...
    void Start()
    {
        ...
        GameObject g = Instantiate(gunModel, gunPoint.transform);
        hand = g.transform;
    ...
    void Update()
    {
        if (!gunPoint) { print($"No Camera Assigned to [{this}]!"); return; } //print warning if no camera is found

        aimDir = ((gunPoint.transform.position + (gunPoint.transform.forward * 99999)) - hand.transform.position).normalized; // sets the initial trajectory of the projectile
